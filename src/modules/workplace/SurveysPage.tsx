...
const SurveysPage = () => {
  ...
  const canManage = permissions.canScopedPermission('survey:manage', ['ALL']);
  const canRespond = permissions.canScopedPermission('survey:respond', ['SELF']);
  const canResults = permissions.canScopedPermission('survey:results', [
    'TEAM',
    'DEPARTMENT',
    'ALL',
  ]);
  ...
  useEffect(() => {
    void run(load, 'Surveys are up to date.');
  }, [load, run]);

  const openSurvey = async (id: string, mode: 'admin' | 'respond') => {
    await run(async () => {
      const value = await client.request<{ survey: SurveyDetailRow }>(SurveyDetailDocument, { id });
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-content-primary">Surveys</h1>
              ...
              {available.map((item) => (
                <li key={item.id} className="flex items-center justify-between gap-3 py-3">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={item.completed || item.status !== 'PUBLISHED'}
      ...
      {survey && (
        <Card title={survey.summary.title}>
          ...
          {surveyMode === 'respond' &&
            ...
            survey.summary.status === 'PUBLISHED' && (
              <Button
                className="mt-4"
                busy={busy}
                onClick={() =>
