...
it('blocks submission while holiday dates are loading', () => {
  renderModal({ upcomingHolidaysLoading: true });
  fillValidRequest();
  expect(screen.getByText('Checking holidays')).toBeTruthy();
  expect(
    screen.getByRole<HTMLButtonElement>('button', { name: 'Submit Application' }).disabled
  ).toBe(true);
  fireEvent.submit(getApplyLeaveForm());
  expect(request).not.toHaveBeenCalled();
});

it('shows holiday failure, retries it, and blocks submission', () => {
  const onRetryUpcomingHolidays = vi.fn();
  renderModal({
    upcomingHolidaysFailure: 'Holiday service is unavailable.',
    onRetryUpcomingHolidays,
  });
  fillValidRequest();
  expect(screen.getByText('Holiday dates could not be loaded')).toBeTruthy();
  fireEvent.click(screen.getByRole('button', { name: 'Try again' }));
  expect(onRetryUpcomingHolidays).toHaveBeenCalledOnce();
  fireEvent.submit(getApplyLeaveForm());
  expect(request).not.toHaveBeenCalled();
});
