...
it('does not publish a superseded response after a newer search completes', async () => {
  ...
  await act(async () => {
    current.resolve(managedPage([{ ...ashaRow, id: 'attendance-43', employeeName: 'Bina Shah' }]));
    await Promise.resolve();
  });
  expect(screen.getAllByText('Bina Shah')[0]).toBeTruthy();
  await act(async () => {
...
it('fails closed across a client replacement and ignores the old page request', async () => {
  ...
  expect((replacementClient.request.mock.calls as unknown[][])[0][1]).not.toHaveProperty(
    'employeeId'
  );
